...
public class MyTcpServer {
    ...
    public MyTcpServer(String host, int port) {
        try {
    ...
    public void connect() throws IOException {
        ...
        var replyConnectionRequest =
                new MyTcpPacket.Builder()
                        .withPacketType(PacketType.SYN)
                        .withSequenceNum(firstConnectionResponse.getSequenceNum())
                        .withPeerAddress(destAddress)
                        .withPeerPort(destPort)
                        .build();

        sendPacket(replyConnectionRequest);
        var secondConnectionResponse = receivePacket();
        if (!secondConnectionResponse.getPacketType().equals(PacketType.SYN_ACK)) {
